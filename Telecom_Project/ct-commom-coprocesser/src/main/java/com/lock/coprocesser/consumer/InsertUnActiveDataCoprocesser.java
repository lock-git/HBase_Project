...
public class InsertUnActiveDataCoprocesser extends BaseRegionObserver {
    ...
    public void postPut(ObserverContext<RegionCoprocessorEnvironment> e,
        ...
        if ("1".equals(flag)) { // 主要主叫数据插入才会触发协处理器
            ...
            unActivePut.addColumn(Bytes.toBytes(Names.TABLE_FAMILY_UNACTIVE.value()), Bytes.toBytes("call1"),
                    Bytes.toBytes(call2));
            unActivePut.addColumn(Bytes.toBytes(Names.TABLE_FAMILY_UNACTIVE.value()), Bytes.toBytes("calltime"),
                    Bytes.toBytes(calltiem));
            unActivePut.addColumn(Bytes.toBytes(Names.TABLE_FAMILY_UNACTIVE.value()), Bytes.toBytes("call1"),
                    Bytes.toBytes(call1));
            unActivePut.addColumn(Bytes.toBytes(Names.TABLE_FAMILY_UNACTIVE.value()), Bytes.toBytes("duration"),
                    Bytes.toBytes(duration));
            unActivePut.addColumn(Bytes.toBytes(Names.TABLE_FAMILY_UNACTIVE.value()), Bytes.toBytes("flag"),
                    Bytes.toBytes("0"));

            try {
                Table t = e.getEnvironment().getTable(TableName.valueOf(Names.TABLE_CALLLOG.value()));
                t.put(unActivePut);
